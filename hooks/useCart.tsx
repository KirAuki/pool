...
export const CartContextProvider = (props: Props) => {
    ...
    const handleRemoveProductFromCart = useCallback(( product: CartProductType) => {
        if(cartProducts){
            ...
            toast.success('Товар удален' , {id: 'removefromcart',})
            localStorage.setItem('PoolShopCartItems', JSON.stringify(filteredProducts))
        }
    } , [cartProducts])
    ...
    const handleCartQtyDecrease = useCallback((product : CartProductType) => {
        ...
        if(product.quantity === 1){
            return toast.error('Упс это минимум')
        }
        if(cartProducts) {
    ...
    const handleCartQtyIncrease = useCallback((product : CartProductType) => {
        ...
        if(product.quantity === 99){
            return toast.error('Упс это максимум')
        }
        if(cartProducts) {
            updatedCart = [...cartProducts]
    ...
    const handleClearCart = useCallback(() => {
        ...
        setCartTotalQty(0)
        localStorage.setItem('PoolShopCartItems', JSON.stringify(null))
    },[])
